# TODO: We should clarify that we are supposed to run only once a day
...
# TODO: This could be taken put, saved for reference
# Pre-backup checks
...
# TODO: For sanity we should check that we have not enabled both plan text backups and custom backups

# TODO: We should pre-flight check that there is a path added to the backup folder in config, otherwise we can exit
...
function New-PgBackup {
  ...
  # TODO: We should definately check that we can find the PostgreSQL commands
  ...
  if ($ENABLE_GLOBALS_BACKUPS -eq 'yes') {
    ...
    $GlobalsBackupFile = Join-Path $FinalBackupDir 'globals.sql'
    pg_dumpall -g -h $HOSTNAME -U $USERNAME | Out-File -FilePath $GlobalsBackupFile -Encoding utf8 -Force; Compress-Archive -Path $GlobalsBackupFile -DestinationPath "$GlobalsBackupFile.zip" -Force; Remove-Item $GlobalsBackupFile
  }
  
  # Schema-only backups
  # We only need to do schema backups if there is any database names added in config
  if ($SchemaOnlyDatabases -ne "") {
    ...
    foreach ($Database in $SchemaOnlyDatabases) {
      ...
      $SchemaBackupFile = Join-Path $FinalBackupDir "${Database}_SCHEMA.sql"
  ...
  foreach ($Database in $FullDatabases) {
    if ($ENABLE_PLAIN_BACKUPS -eq 'yes') {
      ...
      $PlainBackupFile = Join-Path $FinalBackupDir "${Database}.sql"
    ...
    if ($ENABLE_CUSTOM_BACKUPS -eq 'yes') {
      ...
      $CustomBackupFile = Join-Path $FinalBackupDir "${Database}.bak"
